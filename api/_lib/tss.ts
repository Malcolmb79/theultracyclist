...
// scaled so a maximal 1-hour effort right at FTP scores exactly 100.
// Needs weighted average power (Strava's Normalized-Power equivalent, the
// right input for TSS rather than plain average since it accounts for
// how variable the effort was) and the athlete's own FTP - returns null
// rather than a misleading number if either is missing (no power meter on
// that ride, or FTP never set).
