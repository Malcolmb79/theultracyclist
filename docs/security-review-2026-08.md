# Security review â€” 3 August 2026
...
| | |
|---|---|
| **High** | 1 â€” the live-page visibility toggle does not cover the raw feeds |
| **Medium** | 1 â€” no security response headers |
| **Low** | 2 â€” no rate limiting; `strava-activities` is public |
...
## HIGH â€” "Show live page to visitors" doesn't hide the position feeds
...
|---|---|---|
| `api/live-tracker.ts` | partly â€” see below | position, 3000 track points |
...
`configured: false` when hidden, which stops the page drawing anything â€” but it
...
the route begins and ends near home â€” established earlier when the
...
Redis fails closed instead of reading as "no config, therefore visible" â€”
...
page was hidden would be cached and then handed to every visitor â€” the fix
becoming the leak. Owner responses are therefore `private, no-store`.

---

## MEDIUM â€” No security response headers
...
- **No CSP.** The strongest mitigation against XSS, which matters because the
  ...
  it isn't currently exploitable â€” but CSP is what contains the class rather
  than the instance.
...
- `script-src 'self' https://www.instagram.com https://*.cdninstagram.com` â€”
...
- `img-src ... https:` and `connect-src 'self' https:` are the loose ones, on
  ...
  didn't predict â€” during the attempt, silently. Weather, reverse geocoding and
...
The residual risk is cost rather than compromise â€” an uncached route being
hammered bills Vercel and Neon.

## LOW â€” `api/history.json.ts` has no consumer
...
## LOW â€” `api/strava-activities.ts` is unauthenticated
...
**SQL injection â€” not present.** Every query in `api/_lib/trackerDb.ts` uses
...
**XSS â€” no sinks.** No `dangerouslySetInnerHTML`, no `innerHTML`, anywhere in
...
**Secrets â€” correctly handled.** The ingest token and the Connect IQ signing key
...
**Session design â€” sound.** HMAC-signed, `HttpOnly`, `Secure`, `SameSite=Lax`,
...
**Authorisation â€” correctly tiered.** Microsoft sign-in is allowlisted by email.
...
**Webhook verification â€” correct.** `api/whatsapp-webhook.ts` validates the
...
1. ~~Close the visibility gap~~ â€” done, across all three endpoints.
2. ~~Add security headers~~ â€” done; verify the two embeds on the deployed site.
